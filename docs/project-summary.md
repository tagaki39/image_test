...
| Full Tests 耗时 | ~73-83s（含 1 次完整 AI 生成） |
| 流水线阶段 | 5 个（含报告归档） |
...
>
> - 基于 pytest + requests 搭建接口自动化框架，覆盖鉴权、异常参数、异步任务轮询、结果校验 6 类场景，8 条用例；分层架构（api/services/tests）+ fixture 依赖注入 + JSON 参数化数据驱动
